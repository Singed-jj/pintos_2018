  ...
  for (hist_idx = 0; hist_idx < sizeof histogram / sizeof *histogram;
       hist_idx++)
    {
      while (histogram[hist_idx]-- > 0) 
        {
//  printf("%s buf2 : %zu, hist_idx : %zu \n",__func__,buf2[buf_idx],hist_idx);
// ASSERT(buf2[buf_idx] != hist_idx);
          if (buf2[buf_idx] != hist_idx)
            fail ("bad value %d hist_idx %d in byte %zu", buf2[buf_idx],hist_idx, buf_idx);
          buf_idx++;
        } 
    }
