...
process_execute (const char *file_name) 
{
  ...
  //printf("exec proc\n"); 
  char *file_name_r, *next;
  char str[strlen(file_name)+1];
  
  strlcpy(str, file_name, strlen(file_name)+1);

  file_name_r = strtok_r(str, " ", &next);
  //printf("fnr : %s", file_name);
...
start_process (void *f_name)
{
  ...
  char * fn_copy1 = malloc(strlen(f_name)+1);
  char * fn_copy2 = malloc(strlen(f_name)+1);
...
void
argument_pass (char **parse, int count, void **esp){
  int i;
  
  int * argv = calloc(count, sizeof(int));
    
  //printf("argument_pass11\n"); 
  for (i=count-1; i>-1; i--){
    *esp -= strlen(parse[i]) + 1;
    memcpy(*esp, (char *)parse[i], strlen(parse[i])+1);
    argv[i] = *esp;
  }

  //printf("argument_pass22\n"); 
  while((int)*esp%4 != 0)
  {
    *esp -= 1;
    uint8_t wa = 0;
  ...
  //printf("argument_pass33\n"); 
  int arg4 = 0;

  *esp -= 4;
  memcpy(*esp, &arg4, sizeof (int));

  //printf("argument_pass44\n"); 
  for(i=count-1;i>=0;i--)
  {
    *esp -= 4;
    memcpy(*esp, &argv[i], 4);
  ...
  int arg_start = *esp;
  *esp -= 4;
  memcpy(*esp, &arg_start, 4);

  //printf("argument_pass55\n"); 
  *esp -= 4;
  memcpy(*esp, &count, 4);

  *esp -= 4;
  int fake_return = 0;
  memcpy(*esp, &fake_return, 4);

  //printf("argument_pass66\n"); 
  free(argv);
}
...
process_wait (tid_t child_tid) 
{
  ...
  //t->wc_tid = child_tid;
  //printf("just before end of process_wait %d\n",child_tid); 
  //child->running = true;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char * file_name_r = malloc (strlen(file_name)+1);
  strlcpy(file_name_r, file_name, strlen(file_name)+1);
...
setup_stack (void **esp) 
{
  ...
  #ifdef VM
      frame_lock_acquire();
      kpage = frame_palloc_get_page(PAL_ZERO,((uint8_t *)PHYS_BASE)-PGSIZE);
  #else
  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
        *esp = PHYS_BASE;
    #ifdef VM
        page_create(kpage,((uint8_t *) PHYS_BASE) - PGSIZE);
        frame_lock_release();
    #endif
