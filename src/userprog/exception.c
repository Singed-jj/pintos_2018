...
/*
     printf("thread esp : %p\n", thread_current()->esp);
           printf("interrupt esp : %p\n", f->esp);
    printf("faulted addr : %p\n", fault_addr);
  */
 printf(" checkpoint ; page_fault : \n");
...
//    printf("frame_lock_aquiered and upage: %p \n",upage);
//    printf("at the first time interrupt esp : %p\n", f->esp);
    struct thread * t = thread_current();
    struct page *page = page_find (&t->page_table, upage);
    if (page != NULL){
//    printf("here not present  %d \n",page->swap_index);

      if (page ->swap_index != -1){
       // printf("swap in is called \n");
       // printf("list size%d\n;",list_size(&frame_list));
        void *f = frame_palloc_get_page(PAL_USER, upage);
        swap_in(page->swap_index, f);
        ...
        pagedir_set_dirty(t->pagedir, page->upage, true);

        if(install_page(upage,f,true)){
          //printf("swap in completed!!!!\n");
          frame_lock_release();
          return;
        }else{
    ...
    // case 3 file doesn't exist
      ...
      if (f->esp < PHYS_BASE - MAX_STACK_SIZE){
           //printf("thread esp : %p\n", thread_current()->esp);
           //printf(" kernel !!!!!!!!\n");
         return;
      }
      ...
      if(is_user_vaddr(fault_addr) && (
		...
		(uint8_t *) t->esp -32 <= (uint8_t *) fault_addr)
        ){
...
//  printf("%s : kpage : %p\n",__func__, kpage);     
    ...
    //printf("frame_lock_released but not good \n");
          frame_lock_release();
          return;
        }
